CREATE Procedure [ADP].[CDB_Adoption_Associate_MailRecipientlist]  
...
WHERE [Type]=1 And SBU='CDB' AND ID NOT IN (27,57)
...
USING  #TempCDB S ON 
T.EmployeeID=S.EmployeeID AND T.Type=1 AND T.SBU='CDB'
WHEN MATCHED THEN
UPDATE SET IsDeleted=0
WHEN NOT MATCHED THEN
INSERT (EmployeeID,EmailID,Type,SBU,IsDeleted)
VALUES (S.EmployeeID,S.EmailID,1,'CDB',0);
select (STUFF((SELECT distinct ',' +  
  
 EmailID from  [Adp].[MailRecipient_Associate] where [Type]=1  and SBU='CDB' and isdeleted='0'
     ...
     ,(STUFF((SELECT distinct ',' +  
        ...
        EmailID from  [Adp].[MailRecipient_Associate] where [Type]=2  and SBU='CDB' and isdeleted='0'
 ...
 EmailID from  [Adp].[MailRecipient_Associate] where [Type]=3  and SBU='CDB' and isdeleted='0'
