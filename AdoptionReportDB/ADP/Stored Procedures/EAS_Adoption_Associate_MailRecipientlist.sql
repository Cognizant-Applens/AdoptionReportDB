CREATE Procedure [ADP].[EAS_Adoption_Associate_MailRecipientlist]  
...
WHERE [Type]=1 AND SBU='EAS' AND ID NOT IN (58)
...
USING  #TempEAS S ON 
T.EmployeeID=S.EmployeeID AND T.Type=1 AND T.SBU='EAS'
WHEN MATCHED THEN
UPDATE SET IsDeleted=0
WHEN NOT MATCHED THEN
INSERT (EmployeeID,EmailID,Type,SBU,IsDeleted)
VALUES (S.EmployeeID,S.EmailID,1,'EAS',0); 
select (STUFF((SELECT distinct ',' +  
  
 EmailID from  [Adp].[MailRecipient_Associate] where [Type]=1  and SBU='EAS' and isdeleted='0'
     ...
     ,(STUFF((SELECT distinct ',' +  
        ...
        EmailID from  [Adp].[MailRecipient_Associate] where [Type]=2  and SBU='EAS' and isdeleted='0'
 ...
 EmailID from  [Adp].[MailRecipient_Associate] where [Type]=3  and SBU='EAS' and isdeleted='0'
...
END TRY  
 ...
 SELECT @ErrorMessage = ERROR_MESSAGE()  
  --INSERT Error      
  EXEC [AppVisionLens].[dbo].AVL_InsertError '[dbo].[EAS_Adoption_Associate_MailRecipientlist]', @ErrorMessage, '',''  
 ROLLBACK TRAN
 RETURN @ErrorMessage  
  END CATCH     
