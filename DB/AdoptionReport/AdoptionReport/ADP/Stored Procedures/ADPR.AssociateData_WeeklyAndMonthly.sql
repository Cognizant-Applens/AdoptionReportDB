USE [AdoptionReport]
GO
/****** Object:  StoredProcedure [ADPR].[AssociateData_WeeklyAndMonthly]    Script Date: 11/3/2023 12:49:31 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER procedure [ADPR].[AssociateData_WeeklyAndMonthly] --'ADM','Week'
(
...
--select distinct a.ESAProjectID
--from [ADP].[Input_Data_AssociateRAW](NOLOCK) a  
--left join [Adp].[CentralRepository_ActiveAllocations](NOLOCK) b  on a.ESAProjectID=B.Project_ID 
--WHERE (B.Allocation_Start_Date BETWEEN '2023-10-01' AND '2023-10-25')

-- SELECT * FROM  [ADP].[Associate_Projects] WHERE ESAProjectID ='1000360404'
...
,ISNULL(SUM(Workdays / (CONVERT(DECIMAL(5, 2), 23)) * Allocation_Percentage), 0) AS FTE_Percentage,              
ISNULL(SUM((Workdays / CONVERT(DECIMAL(5, 2),23)) * Allocation_Percentage) / 100, 0) AS ESA_FTE_Count             
...
INSERT INTO #Associalte_FinalAllocation         
 ...
 INNER JOIN [ADPR].[Input_Data_AssociateRAW] C ON a.project_id = c.EsaProjectID       
...
LEFT JOIN [ADPR].[Input_Data_AssociateRAW] AD ON PM.EsaProjectID = ad.EsaProjectID      
...
CRS.[Designation]     
 ...
 LEFT JOIN #Associalte_Final_All AF ON PM.EsaProjectID = AF.Project_ID      
...
INSERT INTO #Associate_Summary         
 ...
 JOIN [ADPR].[Input_Data_AssociateRAW] BU ON af.EsaProjectID = bU.EsaProjectID      
 LEFT JOIN #Total_Effort B ON af.Parent_Accountid = b.Parent_Accountid      
...
a.Parent_Accountid      
 ...
 LEFT OUTER JOIN [ADPR].[Associate_Allocation_Raw] dp ON A.EmployeeID = dp.Associate_ID and a.Parent_Accountid=dp.Parent_Accountid and A.EsaProjectID=dp.Project_ID AND a.SBU=dp.SBU      
   ...
   --SELECT * FROM #AssociateActual_Final ORDER BY Parent_Accountid,EsaProjectid,EmployeeID  
...
--SELECT DISTINCT EsaProjectid FROM #AssociateActual_Final_All_Grade

--SELECT DISTINCT EsaProjectid FROM [ADP].[Associate_Projects]
...
SELECT DISTINCT      
  ...
  ,ISNULL((SELECT SUM(Avaialble_FTE_Below_M) FROM #Associate_BUcompliance B   
  WHERE A.SBU=B.SBU AND A.EmployeeID = B.EmployeeID AND Project_Scope='AD' AND B.[Associate_BU_Compliance] >50 and B.[Associate_BU_Compliance]  <=80),0)  
  ,ISNULL((SELECT SUM(Avaialble_FTE_Below_M) FROM #Associate_BUcompliance B   
...
--DROP TABLE #Temp_BM_Applns_Below_M    
...
--DROP TABLE #ScopeBU  
--DROP TABLE #SBU_Compliance_AVM_Cal  
--DROP TABLE #SBU_Compliance_AVM  
--DROP TABLE #BUDATA_AVM  
--DROP TABLE #SBU_Compliance_AD_Cal  
--DROP TABLE #SBU_Compliance_AD  
--DROP TABLE #BUDATA_AD  
--DROP TABLE #SBU_Compliance_INTEG_Cal  
--DROP TABLE #SBU_Compliance_INTEG  
--DROP TABLE #BUDATA_INTEG  
--DROP TABLE #Associate_BUcompliance_all  
--DROP TABLE #SBU_Compliance_Cal  
...
INSERT INTO #Associate_Summary_All_Grade        
 ...
 JOIN [ADPR].[Input_Data_AssociateRAW] BU ON af.EsaProjectID = bU.EsaProjectID      
 LEFT JOIN #Total_Effort_All_Grade B ON af.Parent_Accountid = b.Parent_Accountid      
  AND af.Parent_AccountName = b.Parent_AccountName      
  AND af.EsaProjectID = b.EsaProjectID      
  AND af.EmployeeID = b.EmployeeID    
...
MAS_Effort ,      
Actual_Effort ,      
Associate_Account_Compliance  FROM #AssociateActual_Final_All_Grade   
...
SELECT DISTINCT a.SBU,a.EmployeeID,a.Department_Name ,sum(a.Avaialble_FTE_Below_M),sum(a.Available_Hours),      
sum(a.MPS_Effort),sum(a.Work_Profile_AD),Sum(a.MAS_Effort),sum(a.Actual_Effort),ISNULL(((SUM(a.Actual_Effort)) / NULLIF(SUM(a.Available_Hours), 0) * 100), 0) from #AssociateActual_Final_All_Grade A      
JOIN [ADPR].[Input_Data_AssociateRAW] C ON a.EsaProjectID=C.EsaProjectID       
where C.DE_Inscope='In scope'      
GROUP BY SBU,EmployeeID,Department_Name    
...
[cpcinchpv004140].[DiscoverEDS].[EDS].[TimesheetDetail_All]  
...
group by EsaProjectID,submitterid
 ...
 select A.EsaProjectID,a.submitterid,actual
,CASE WHEN isnull(MHC.Mandatory_Hours,0)=0    ----cibi
  THEN    
  ISNULL((l.Diff * l.Allocation_Percentage *   
  CASE WHEN Ltrim(Rtrim(LC.country))='IND' AND Ltrim(Rtrim(LC.City)) NOT IN ('Kolkata','Noida')  
  THEN   
  @IndiaHours ELSE @NonIndiaHours END)/100 , 0)     
  ELSE    
   ISNULL((l.Diff * l.Allocation_Percentage * MHC.Mandatory_Hours)/100 , 0)        
   END As Available_Hours
   into #tempfinal_available
from #EDSTimesheet a
join #FTE_count_Loc l on l.Project_ID=A.EsaProjectID and l.associate_id=a.submitterid    
LEFT join [ADP].[MandatoryHoursConfig] MHC on MHC.EsaProjectID=A.EsaProjectID  --and MHC.Isdeleted=0    
LEFT JOIN [AppVisionLens].esa.locationmaster LC on l.FTE_Location_cur=LC.Assignment_Location 
  GROUP BY A.EsaProjectID,A.actual,a.submitterid,--a.[TimesheetSubmissionDate],
  l.Allocation_Startdate ,l.Allocation_enddate,l.Allocation_Percentage  ,l.FTE_Location_cur,LC.City,LC.country  
    ,MHC.Mandatory_Hours 	,	l.Diff	
...
------New FTE calculation Start-------- 

IF OBJECT_ID(N'tempdb..#Eligible_FTE') IS NOT NULL BEGIN DROP TABLE #Eligible_FTE END  
CREATE table #Eligible_FTE  
(  
SBU nvarchar(100),
...
GA.[Allocation_End_Date],
GA.[Allocation_Percentage],GA.Location 
FROM [AppVisionLens].DBO.[VW_Applens_OPL_Adoption_Eligible_projects](NOLOCK)  T  
...
IF OBJECT_ID(N'tempdb..#Associate_WeekDays_Prj') IS NOT NULL BEGIN DROP TABLE #Associate_WeekDays_Prj END    
CREATE TABLE #Associate_WeekDays_Prj  
...
DayWeek VARCHAR(100)  
  ...
  IF OBJECT_ID(N'tempdb..#Associate_days_Prj') IS NOT NULL BEGIN DROP TABLE #Associate_days_Prj END    
CREATE table #Associate_days_Prj  
...
DECLARE @id_first_Prj int  
  
DECLARE @id_last_Prj int  
  
DECLARE @id_first_Date_prj date  
  
DECLARE @id_Last_date_Prj date  
  
DECLARE @As_Datepart1_Prj INT  
  
DECLARE @As_Datepart2_Prj INT  
  
DECLARE @As_Datepart3_Prj INT  
  
DECLARE @AL_Date_Prj date  
  
DECLARE @AssociateId_Prj varchar(10)  
  
DECLARE @FTE_Location_Prj nvarchar(20)  
  
    
DECLARE @Project_id_Prj Char(15)  
   
  
DECLARE @Allocation_startdate_Prj datetime  
  
DECLARE @Allocation_enddate_Prj datetime  
  
DECLARE @allocation_Percentage_grt_Prj  decimal(10,4)  
  
SET @id_first_Prj = (SELECT MIN(id) FROM #Eligible_FTE)  
SET @id_last_Prj = (SELECT MAX(id) FROM #Eligible_FTE)  
  
  

  
WHILE(@id_last_Prj>=@id_first_Prj)  
  
Begin  
  
  
SET @id_first_Date_prj = (SELECT Allocation_Startdate  FROM #Eligible_FTE WHERE Id = @id_first_Prj)  
SET @id_Last_date_Prj = (SELECT Allocation_enddate FROM #Eligible_FTE WHERE Id = @id_first_Prj)  
SET @AL_Date_Prj = @id_first_Date_prj  
SET @AssociateId_Prj = (SELECT Associate_id FROM #Eligible_FTE WHERE Id = @id_first_Prj)  
SET @Project_id_Prj = (SELECT Project_ID FROM #Eligible_FTE WHERE Id = @id_first_Prj AND Associate_id = @AssociateId_Prj)  
SET @Allocation_startdate_Prj = (SELECT Allocation_Startdate FROM #Eligible_FTE WHERE Id = @id_first_Prj AND Associate_id = @AssociateId_Prj)  
SET @Allocation_enddate_Prj = (SELECT  Allocation_enddate FROM #Eligible_FTE WHERE Id = @id_first_Prj AND Associate_id = @AssociateId_Prj)  
SET @allocation_Percentage_grt_Prj = (SELECT  Allocation_Percentage FROM #Eligible_FTE WHERE Id = @id_first_Prj AND Associate_id = @AssociateId_Prj)  
set @FTE_Location_Prj =   (SELECT  FTE_Location FROM #Eligible_FTE WHERE Id = @id_first_Prj AND Associate_id = @AssociateId_Prj)  
SET @As_Datepart1_Prj = DATEPART(dd, @id_first_Date_prj)  
SET  @As_Datepart2_Prj = DATEPART(dd, @EndDate)  
  
WHILE (@As_Datepart2_Prj >= @As_Datepart1_Prj) BEGIN  
  
  
INSERT INTO #Associate_WeekDays_Prj  
  
 SELECT @AL_Date_Prj ,DATENAME(dw, @AL_Date_Prj)  
   
SET @AL_DATE_Prj = DATEADD(DAY, 1, @AL_DATE_Prj)  
  
SET @As_Datepart1_Prj = @As_Datepart1_Prj + 1  
  
  
END  
  
  
DELETE FROM #Associate_WeekDays_Prj WHERE DayWeek IN ('Saturday', 'Sunday')  
  
  
INSERT INTO #Associate_days_Prj  
 SELECT  
  @Project_id_Prj  
  ,@AssociateId_Prj  
  ,@Allocation_startdate_Prj  
  ,@Allocation_enddate_Prj  
  ,@allocation_Percentage_grt_Prj  
  ,@FTE_Location_Prj 
  ,(SELECT  
    COUNT(1)  
   FROM #Associate_WeekDays_Prj)  
,(SELECT count(holiday) As 'Holidays'   
from  #HOLIDAYLIST where Location=@FTE_Location_Prj  and holiday BETWEEN @Allocation_startdate_Prj AND  @Allocation_enddate_Prj)  
SET @id_first_Prj = @id_first_Prj + 1  
TRUNCATE TABLE #Associate_WeekDays_Prj  
END  
  

IF OBJECT_ID(N'tempdb..#Associate_diff_Prj') IS NOT NULL BEGIN DROP TABLE #Associate_diff_Prj END    
CREATE table #Associate_diff_Prj  
...
Insert into #Associate_diff_Prj  
  
select DISTINCT Project_id , Associate_id, Allocation_Startdate,Allocation_Enddate,Allocation_Percentage,FTE_Location,Workdays, Isnull(Workdays-Holidays,0)  
from #Associate_days_Prj  

--IF OBJECT_ID(N'tempdb..#EligibleFTE') IS NOT NULL BEGIN DROP TABLE #EligibleFTE END  
--select distinct a.SBU_Delivery as 'SBU',a.ESAProjectID,
--C.Workdays_diff,b.Allocation_Percentage
--into #EligibleFTE 
--from [AppVisionLens].DBO.[VW_Applens_OPL_Adoption_Eligible_projects](NOLOCK) a  
--left join [Adp].[CentralRepository_ActiveAllocations](NOLOCK) b  on a.ESAProjectID=B.Project_ID 
--left join #Associate_diff_Prj(nolock) c on a.ESAProjectID=c.Project_id
--WHERE B.Allocation_Start_Date between  @StartDate and @Enddate 

INSERT INTO #EligibleProjects
select distinct b.SBU,A.Project_id,
ISNULL(SUM((a.Workdays_diff / CONVERT(DECIMAL(5, 2), 20)) * a.Allocation_Percentage) / 100, 0) AS ESA_FTE_Count 
from #Associate_diff_Prj a 
left join #Eligible_FTE b on a.Project_id = b.Project_ID and a.Associate_id = b.Associate_id
group by b.SBU, A.Project_id

DELETE FROM #Associate_WeekDays_Prj WHERE DayWeek IN ('Saturday', 'Sunday')  

---ADM adoption onboard projects
...
select distinct c.sbu,c.Project_ID,c.ESA_FTE_Count --into #OnboardedProjects  
...
join #EligibleProjects(NOLOCK) c on c.Project_ID=a.esaprojectid  
where b.ScreenID=4 and b.CompletionPercentage=100  
AND A.EsaProjectID NOT IN 
(SELECT  EsaProjectID FROM #ExemptionProjects)


--SELECT * FROM [AppVisionLens].[AVL].[PRJ_ConfigurationProgress]
--SELECT distinct Project_ID FROM #EligibleProjects
--SELECT distinct Project_ID FROM #OnboardedProjects
...
-- INSERT INTO #EligibleProjects
--	select distinct a.SBU,A.Project_ID,A.ESA_FTE_Count --into #EligibleProjects  
--	from [ADP].[Associate_Allocation_Raw](NOLOCK) a  join  
--	[AppVisionLens].DBO.[VW_Applens_OPL_Adoption_Eligible_projects](NOLOCK) b   
--	on a.Project_ID=B.ESA_Project_ID group by a.SBU,A.Project_ID,A.ESA_FTE_Count
--END 
...
BEGIN
	...
	--EPS
	select distinct a.SBU,A.Project_ID,A.ESA_FTE_Count --into #EligibleProjects  
	from [ADPR].[Associate_Allocation_Raw](NOLOCK) a  join  
	[ADP].[Associate_Projects_EAS](NOLOCK) b   
	on a.Project_ID=B.EsaProjectID group by a.SBU,A.Project_ID,A.ESA_FTE_Count 


---EPS adoption onboard projects
...
(NOLOCK) a  join  
	[ADP].[Associate_Projects_AIA](NOLOCK) b   
	on a.Project_ID=B.EsaProjectID group by a.SBU,A.Project_ID,A.ESA_FTE_Count  


---AIA adoption onboard projects
...
BEGIN
	...
	from [ADPR].[Associate_Allocation_Raw]
(NOLOCK) a  join  
	[ADP].[Associate_Projects_CDB](NOLOCK) b   
	on a.Project_ID=B.EsaProjectID group by a.SBU,A.Project_ID,A.ESA_FTE_Count
	
---CDBI adoption onboard projects
...
--from [AppVisionLens].avl.mas_projectmaster(nolock) a join   
--[AppVisionLens].[AVL].[PRJ_ConfigurationProgress](nolock)b on a.projectid=b.projectid  
--join [ADP].[Associate_Allocation_Raw](NOLOCK) c on c.Project_ID=a.esaprojectid  
--where b.ScreenID=4 and b.CompletionPercentage=100  
...
from  #EligibleFinal A  
 join #OnboardFinal B on A.SBU=B.SBU  
...
join #SBU_Compliance_Below_M b on a.SBU = b.SBU   
join #FinalCal c on a.SBU=c.sbu  
...
INSERT INTO #SBUComplianceTotal      
 SELECT DISTINCT      
  ...
  ,SUM(CONVERT(DECIMAL(10,2), [ESA TS Compliance (Overall)])) AS 'ESA TS Compliance (Overall)'     
...
FROM #Project_Compliance 
 ...
 --SELECT * FROM #Project_Compliance ORDER BY Parent_Accountid,EsaProjectid  
...
FROM #Account_Compliance_INTEG
 ...
 SELECT @ErrorMessage = ERROR_MESSAGE()    
  --INSERT Error        
  EXEC [AppVisionLens].[dbo].AVL_InsertError '[dbo].[Adp_AssociateData_Monthly] ', @ErrorMessage, '',''    
  RETURN @ErrorMessage    
  END CATCH       
