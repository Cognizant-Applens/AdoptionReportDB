Create procedure [Adp].[AssociateData_Monthly]  
...
SDM_ID varchar(20) null,  
  
  
SDM_Name varchar(100) null,  
  
  
Practice_Owner varchar(50) null,  
  
  
SDD_ID varchar(20) null,  
...
INSERT INTO #TmpBasicData  
 ...
 FROM Adp.Input_Data_AssociateRAW AC  
  
 INNER JOIN [$(AppVisionLens)].ESA.ProjectAssociates EP  
  ON EP.ProjectID = AC.EsaProjectID  
  
 INNER JOIN [$(AppVisionLens)].ESA.BUAccounts BUA  
  ON BUA.AccountID = ep.ACCOUNT_ID  
  
 LEFT OUTER JOIN [$(AppVisionLens)].AVL.MAS_ProjectMaster PM  
  ON EP.ProjectID = PM.EsaProjectID  
  
  --AND PM.IsProjectsetupcompleted = 'Y'   
  
  AND PM.IsDeleted = 0  
  AND PM.IsESAProject = 1 --and (PM.IsMigratedFromDART in (1)  
  
   
 ORDER BY AC.PracticeOwner, BUA.ACCOUNTNAME;  
...
GA.Location  
 ...
 AND ga.[Allocation_Start_Date] <= @enddate  
  
  
--select * from #FTE_GRT where associate_id='606244'  
...
INSERT INTO #Associate_days  
 ...
 SELECT  
  ...
  ,@FTE_Location   
  ,(SELECT COUNT(*) FROM #Associate_WeekDays)  
...
select  Project_id , Associate_id, Allocation_Startdate,Allocation_Enddate,Allocation_Percentage,FTE_Location,Workdays
, Isnull(Workdays-Holidays,0)  from #Associate_days    
...
INSERT INTO #FTE_Cal_A  
 ...
 SELECT  
  ...
  ,a.FTE_Location,Ltrim(Rtrim(LC.City)) as 'City',Ltrim(Rtrim(LC.country)) as country
  ,a.Workdays_diff  
 ...
 GROUP BY a.Project_id  
    ...
    ,B.Allocation_Startdate  
    ,b.Allocation_enddate  
    ,B.Allocation_Percentage   ,a.FTE_Location,LC.City,LC.country 
    ,a.Workdays_diff  
  
--select * from #FTE_Cal where associate_id='178507'  
...
--select * from #FTE_Final where associate_id='252331'  
...
SELECT Project_ID,associate_id ,Allocation_Startdate ,    
...
--ISNULL(SUM((Diff / CONVERT(DECIMAL(5, 2), @WorkdAYS)) * Allocation_Percentage) / 100, 0) As Workdays_FTE   
...
SELECT Project_ID,associate_id ,Allocation_Startdate ,Allocation_Enddate ,Allocation_Percentage,sum(ESA_FTE_Count), sum(Available_Hours) from #FTE_count_Cal  group by Project_ID,associate_id ,Allocation_Startdate ,Allocation_Enddate ,Allocation_Percentage
...
INSERT into  #FTE_END_Future_LOC  
 ...
 ,@WorkdAYS    
  ,CRS.Location    
  ,Count(holiday)    
  ,ISNULL((@WorkdAYS-Count(holiday)), 0) FROM  [Adp].[Input_Data_AssociateRAW] T   
  
--LEFT JOIN esa.Projects GP ON Convert(varchar,GP.ID) =T.EsaProjectID   
...
, A.FTE_Percentage,  A.ESA_FTE_Count  c,MHC.Mandatory_Hours 
...
SELECT Project_ID,associate_id ,Allocation_Startdate ,Allocation_Enddate ,Allocation_Percentage,sum(ESA_FTE_Count), sum(Available_Hours) from #FTE_END_Future_CAL  group by Project_ID,associate_id ,Allocation_Startdate ,Allocation_Enddate ,Allocation_Percentage    
...
INSERT into  #FTE_MID  
  ...
  SELECT T.EsaProjectID,crs.Associate_ID, crs.Allocation_Start_Date,crs.Allocation_End_Date,crs.Allocation_Percentage ,crs.location  FROM  [Adp].[Input_Data_AssociateRAW] T   
...
INSERT INTO #FTE_MID_Cal    
 ...
 SELECT    
  ...
  ELSE
   ...
   END As Available_Hours 
 --,isnull (sum(B.Workdays_Loc_end * A.ESA_FTE_Count ) *8, 0) As Available_Hours  
...
--select * from #FTE_MID_Final where associate_id='178507'  
 -----------------------------------  
...
INSERT INTO #FTE_ENDATE  
 ...
 SELECT DISTINCT  
  ...
  ,crs.Allocation_Percentage,CRS.Location  
 FROM [Adp].[Input_Data_AssociateRAW] T  
...
SET @AL_Date_1 = @id_first_Date_1--04  
...
IF @Allocation_startdate1 >= @StartDate BEGIN  
...
INSERT INTO #Associate__endate_diff_loc  
 ...
 SELECT  
  ...
  ,(SELECT  
    COUNT(*)  
   FROM #Associate_endate_WeekDays)  
   ,(SELECT count(holiday) As 'Holidays' from  #HOLIDAYLIST     
...
--select * from #Associate__endate_diff where associate_id='414574'  
...
select  Project_id , Associate_id, Allocation_Startdate,Allocation_Enddate,Allocation_Percentage, FTE_Location_END,Workdays_loc,Isnull(Workdays_loc-Holidays_end,0)  from #Associate__endate_diff_loc    
    
--select * from #Associate__endate_diff where associate_id='423747'    
...
SELECT  
 * INTO #FTE_BM  
FROM ((SELECT* from #FTE_count) 
UNION ALL (SELECT* FROM #FTE_Final) 
UNION ALL (SELECT * FROM #FTE_END_Future) UNION ALL (SELECT * FROM #FTE_MID_Final)  
 UNION ALL (SELECT* FROM #FTE_End_Final))p  
  
  
  --SELECT  * from #FTE_BM WHERE project_id='1000145808'  associate_id='551455' group by project_ID,Associate_id   
...
INSERT INTO #Associalte_FinalAllocation  
 ...
 INNER JOIN [Adp].Input_Data_AssociateRAW C  
  ...
  --select * from #Associalte_FinalAllocation  
...
INSERT INTO #Associalte_Final_All  
 ...
 SELECT DISTINCT  
  ...
  --DPT.Dept_Desc,DS.JobCode ,DS.JobCodeDescription  
  --,ROW_NUMBER() OVER (PARTITION BY AF.Project_ID, AF.Associate_id, AF.Allocation_StartDate, AF.Allocation_Enddate ORDER BY AF.Project_ID DESC) AS Topp  
  
  
 FROM #Associalte_FinalAllocation AF  
  
 --JOIN Adp.Input_Data_AssociateRAW ADP ON AF.Project_ID=ADP.EsaProjecti  
...
--drop table #Associalte_Final_All 28872  
--select *from #Associalte_FinalAllocation  where associate_id='252331'  
  
--#Associalte_Final_All  
  
--select *from #Associalte_FinalAllocation  where Project_ID='1000012040'  
...
--select COUNT(*) FROM #TEMPR WHERE  EsaProjectID='1000216596' 287345  
--DROP TABLE #TEMPR  
--SELECT * FROM #Temp_BM_Applns WHERE associateid='323477'  
...
FROM (SELECT DISTINCT  	
 ...
 ,AF.Parent_AccountName As ParentCustomerName  	
  ,LG.IsNonESAAuthorized  	
  ,AF.DE_Inscope  	,CRS.[Dept_Name],CRS.[JobCode],CRS.[Designation]
  --,DPT.Dept_Desc  	
  --,DS.Jobcode  	
...
--select count(*)from #Tempfin where employeeid='659975' and esaprojectid='1000004997' 260806  
-- drop table #Tempfin  
...
--WHERE a.EsaProjectID='1000260822' --B.DE_Inscope='In Scope'  
  
--drop table Tempfin   
--select * from #Associalte_Final_All where associate_id='252331'  
...
FROM #Associalte_Final_All --where  associate_id='551455'  
  
--select * from  Adp_Associate_Allocation_Raw where associate_id='323477' ORDER BY SBU  where  project_id='1000012040 '  
...
--select * from #Allocatedassoc #Loginmaster_associate where EmployeeID='775075'  
...
--select * from #Associate_Accountcompliance where employeeid='839634'
...
--select * from #Associate_Total_account_Compliance_all where employeeid='839634'
...
INSERT INTO #Associate_80  
 ...
 GROUP BY Parent_Accountid  
  ...
  --select * from #Associate_Greater80
...
--select * from #Account_Compliance where   Parent_Accountid='2001627'
--select * from #Associate_80 where Parent_Accountid='2001627'
...
SELECT DISTINCT  
  a.Parent_Accountid  
  ,a.Parent_AccountName,
  A.Vertical,A.MarketUnitName,A.BU 
  ,ISNULL(SUM(a.Avaialble_FTE_Below_M), 0)  
  ,ISNULL(SUM(b.ESA_FTE_Zero), 0)  
  ,ISNULL(SUM(c.ESA_FTE_0_25), 0)  
  ,ISNULL(SUM(d.ESA_FTE_25_50), 0)  
 ...
 GROUP BY  a.Parent_Accountid  ,a.Parent_AccountName,
 A.Vertical ,A.MarketUnitName,A.BU  

	--select * from #Associate_Total_account_Compliance where parent_Accountid='2001627'
	--select * from #Associate_80 where Parent_Accountid='2001627'
