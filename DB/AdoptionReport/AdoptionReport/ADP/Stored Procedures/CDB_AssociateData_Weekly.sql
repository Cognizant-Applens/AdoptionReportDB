create Procedure [Adp].[CDB_AssociateData_Weekly]  
...
select @WorkdAYS = (SELECT  
  COUNT(1) AS WorkDays  
 FROM #WeekDays)  
  
  
, @WorkHours = (SELECT  
  (COUNT(1) * 8) AS WorkHours  
 FROM #WeekDays)  
...
INTO #Associalte_Final_AVM FROM [ADP].[Associate_Allocation_Raw] WHERE Department_Name='CDB' OR Department_Name LIKE  '%CDB%' or
...
FROM (SELECT DISTINCT  
 ...
 LEFT JOIN #Associalte_Final_AVM AF  
  ...
  --AND PA.ParentCustomerID = AF.Parent_Accountid  
...
INSERT INTO #MAS_Effort  
 ...
 FROM #Loginmaster_associate A  
 LEFT JOIN [CPCINCHPV004140].[DiscoverEDS].[EDS].[TimesheetDetail] B  
...
DROP TABLE #VERTICALDATA_AVM 
 ...
 SELECT @ErrorMessage = ERROR_MESSAGE()  
  ...
  EXEC [$(AppVisionLens)].dbo.AVL_InsertError '[Adp].[CDB_AssociateData_Weekly]', @ErrorMessage, '',''  
  RETURN @ErrorMessage  
  END CATCH     
