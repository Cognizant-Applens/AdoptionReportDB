CREATE Procedure [ADPR].[ACCOUNT_PROJECT_SUMMARY_RAW]   
(    
@startdate datetime,    
@endDate datetime,    
@mode varchar(200)    
...
FROM [ADPR].[Associate_Allocation_Raw] A
...
SELECT  DISTINCT    
  ...
  Into #Account_AD  
FROM [ADPR].[Account_Compliance_AD] A    
    
left JOIN [ADPR].[Account_Compliance_AVM_AD] B ON a.Parent_Accountid=B.Parent_Accountid AND A.vertical=B.vertical and A.MarketUnitName=B.MarketUnitName    
...
FROM [ADPR].[Account_Compliance_AM] A    
    
left JOIN [ADPR].[Account_Compliance_AVM_AM] B ON a.Parent_Accountid=B.Parent_Accountid AND A.vertical=B.vertical and A.MarketUnitName=B.MarketUnitName    
...
FROM [ADPR].[Account_Compliance_INTEG] A    
    
left JOIN [ADPR].[Account_Compliance_AVM_INTEG] B ON a.Parent_Accountid=B.Parent_Accountid AND A.vertical=B.vertical and A.MarketUnitName=B.MarketUnitName    
...
[Account Effort Compliance% (ADM)],[Account Associate Compliance% (ADM)],'INTEG'  
 ...
 TRUNCATE TABLE [ADPR].[Account_Compliance_Scope]  
  
  
Insert into [ADPR].[Account_Compliance_Scope] ( [Parent_Accountid],[Parent AccountName],[Vertical],[MARKET UNIT NAME])  
  
SELECT distinct  A.[Parent_Accountid],[Parent AccountName],A.[Vertical],[SBU Delivery (PC2Geo mapping)] 
FROM #AccountCompliance_Split A
...
A.[AM Scope - Account Associate Compliance% (ADM)] = B.[Account Associate Compliance% (ADM)]  
 from [ADPR].[Account_Compliance_Scope] A  
...
from [ADPR].[Account_Compliance_Scope] A  
join [ADPR].[Account_Compliance_YETTOSCOPE] B on A.[Parent_Accountid]=b.[Parent_Accountid]  
and a.Vertical=b.vertical and a.[MARKET UNIT NAME]=b.[MARKET UNIT NAME]  
  
select  A.[Parent_Accountid],[Parent AccountName],A.[Vertical],[MARKET UNIT NAME] as 'SBU Delivery (PC2Geo mapping)' ,[Yet to scope project #],A.[Available Hours],  
...
SELECT  A.SBU AS 'SBU Delivery (PC2Geo mapping)' ,Isnull(convert(decimal(10,2),
A.[ESA FTE(Below_M)]),0) AS 'Overall FTE'  
,Isnull(convert(decimal(10,2),C.[Overall #FTE]),0) AS 'AD Overall FTE'  
,Isnull(convert(decimal(10,2),D.[Overall #FTE]),0) AS 'AM Overall FTE'  
,Isnull(convert(decimal(10,2),E.[Overall #FTE]),0) AS 'INTEGRATED Overall FTE'  
,isnull(convert(decimal(10,2),F.[AVM #FTE]),0) AS 'AD ADM FTE'  
,isnull(convert(decimal(10,2),G.[AVM #FTE]),0) AS 'AM ADM FTE'  
,isnull(convert(decimal(10,2),H.[AVM #FTE]),0) AS 'INTEGRATED ADM FTE'  
,isnull(convert(decimal(10,2),B.[AVM FTE(Below_M)]),0) AS 'ADM FTE'  
,isnull(convert(decimal(10,2),A.[Overall #FTE with TSC %=0]),0) AS 'Overall #FTE with TSC %=0'    
,isnull(convert(decimal(10,2),A.[Overall #FTE with TSC %>0 to 25]),0) AS 'Overall #FTE with TSC %>0 to 25'    
,isnull(convert(decimal(10,2),A.[Overall #FTE with TSC %>25 to 50]),0)AS 'Overall #FTE with TSC %>25 to 50'    
,isnull(convert(decimal(10,2),A.[Overall #FTE with TSC %>50 to 80]),0) AS 'Overall #FTE with TSC %>50 to 80'    
,isnull(convert(decimal(10,2),C.[Overall #FTE with TSC %>80]),0) AS 'AD Overall #FTE with TSC %>80'    
,isnull(convert(decimal(10,2),D.[Overall #FTE with TSC %>80]),0) AS 'AM Overall #FTE with TSC %>80'    
,isnull(convert(decimal(10,2),E.[Overall #FTE with TSC %>80]),0) AS 'INTEGRATED Overall #FTE with TSC %>80'    
,isnull(convert(decimal(10,2),A.[ESA FTE 80(Below_M)]),0) AS 'Overall #FTE with TSC %>80'    
,isnull(convert(decimal(10,2),F.[AVM #FTE with TSC %>80]),0) AS 'AD ADM #FTE with TSC %>80'    
,isnull(convert(decimal(10,2),G.[AVM #FTE with TSC %>80]),0) AS 'AM ADM #FTE with TSC %>80'    
,isnull(convert(decimal(10,2),H.[AVM #FTE with TSC %>80]),0) AS 'INTEGRATED ADM #FTE with TSC %>80'    
,isnull(convert(decimal(10,2),B.[AVM FTE 80(Below_M)]),0) AS 'ADM #FTE with TSC %>80'    
,isnull(convert(decimal(10,2),C.[Available Hours]),0)AS 'AD Available Hours (All)'    
,isnull(convert(decimal(10,2),D.[Available Hours]),0)AS 'AM Available Hours (All)'   
,isnull(convert(decimal(10,2),E.[Available Hours]),0)AS 'INTEGRATED Available Hours (All)'   
,isnull(convert(decimal(10,2),a.[Available Hours(Below_M)]),0)AS 'Available Hours (All)'   
,isnull(convert(decimal(10,2),F.[Available Hours]) ,0)AS 'AD Available Hours (ADM)'    
,isnull(convert(decimal(10,2),G.[Available Hours]) ,0)AS 'AM Available Hours (ADM)'    
,isnull(convert(decimal(10,2),H.[Available Hours]) ,0)AS 'INTEGRATED Available Hours (ADM)'    
,isnull(convert(decimal(10,2),B.[Available Hours(Below_M)]) ,0)AS 'Available Hours (ADM)'    
,isnull(convert(decimal(10,2),C.[Actual Effort]),0) as 'AD Actual Effort (All)'    
,isnull(convert(decimal(10,2),D.[Actual Effort]),0) as 'AM Actual Effort (All)'   
,isnull(convert(decimal(10,2),E.[Actual Effort]),0) as 'INTEGRATED Actual Effort (All)'   
,isnull(convert(decimal(10,2),A.[Actual Effort(Below_M)]),0) as 'Actual Effort (All)'   
,isnull(convert(decimal(10,2),F.[Actual Effort]),0) as 'AD Actual Effort (ADM)'    
,isnull(convert(decimal(10,2),G.[Actual Effort]),0) as 'AM Actual Effort (ADM)'   
,isnull(convert(decimal(10,2),H.[Actual Effort]),0) as 'INTEGRATED Actual Effort (ADM)'   
,isnull(convert(decimal(10,2),B.[Actual Effort(Below_M)]),0) as 'Actual Effort (ADM)'   
,ISNULL(convert(decimal(10,2),c.[BU Effort Compliance%(All)]),0) AS 'AD Scope -BU Effort Compliance%(All)'   
,ISNULL(convert(decimal(10,2),d.[BU Effort Compliance%(All)]),0) AS 'AM Scope -BU Effort Compliance%(All)'   
,ISNULL(convert(decimal(10,2),e.[BU Effort Compliance%(All)]),0) AS 'Integrated Scope -BU Effort Compliance%(All)'   
,ISNULL(convert(decimal(10,2),a.[BU_Effort_Compliance_Percent_Below_M]),0) AS 'BU Effort Compliance%(All)'   
,ISNULL(convert(decimal(10,2),c.[Associate Compliance Percent]),0) AS 'AD Scope -BU Associate Compliance% (All)'     
,ISNULL(convert(decimal(10,2),d.[Associate Compliance Percent]),0) AS 'AM Scope -BU Associate Compliance% (All)'     
,ISNULL(convert(decimal(10,2),e.[Associate Compliance Percent]),0) AS 'Integrated Scope -BU Associate Compliance% (All)'     
,ISNULL(convert(decimal(10,2),a.[Associate_Compliance_Percent_Below_M]),0) AS 'BU Associate Compliance% (All)'  
,ISNULL(convert(decimal(10,2),F.[BU Effort Compliance%(AVM)]),0) AS 'AD Scope-BU Effort Compliance%(ADM)'  
,ISNULL(convert(decimal(10,2),G.[BU Effort Compliance%(AVM)]),0) AS 'AM Scope -BU Effort Compliance%(ADM)'    
,ISNULL(convert(decimal(10,2),H.[BU Effort Compliance%(AVM)]),0) AS 'Integrated Scope -BU Effort Compliance%(ADM)'    
,ISNULL(convert(decimal(10,2),B.[BU_Effort_Compliance_Percent_Below_M]),0) AS 'BU Effort Compliance%(ADM)'    
,ISNULL(convert(decimal(10,2),F.[AVM_Associate_Compliance_Percent]),0)  AS 'AD Scope-BU Associate Compliance% (ADM)'    
,ISNULL(convert(decimal(10,2),G.[AVM_Associate_Compliance_Percent]),0)  AS 'AM Scope -BU Associate Compliance% (ADM)'    
,ISNULL(convert(decimal(10,2),H.[AVM_Associate_Compliance_Percent]),0)  AS 'Integrated Scope -BU Associate Compliance% (ADM)'    
,ISNULL(convert(decimal(10,2),B.[Associate_Compliance_Percent_Below_M]),0)  AS 'BU Associate Compliance% (ADM)'    
...
END TRY    
 ...
 SELECT @ErrorMessage = ERROR_MESSAGE()    
  --INSERT Error        
  EXEC [AppVisionLens].dbo.AVL_InsertError '[ADPR].[ACCOUNT_PROJECT_SUMMARY_RAW]  ', @ErrorMessage, '',''    
  RETURN @ErrorMessage    
  END CATCH       
