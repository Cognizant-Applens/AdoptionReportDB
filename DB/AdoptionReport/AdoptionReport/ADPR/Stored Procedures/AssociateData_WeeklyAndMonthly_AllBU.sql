CREATE   PROCEDURE [ADPR].[AssociateData_WeeklyAndMonthly_AllBU] --'ADM','Week'  
(  
...
IF OBJECT_ID(N'tempdb..#Associate_Final_AVM_Below_M') IS NOT NULL  BEGIN DROP TABLE #Associate_Final_AVM_Below_M END  
  
CREATE TABLE #Associate_Final_AVM_Below_M (  
Parent_Accountid char(50)  
,Parent_AccountName varchar(100)  
,SBU varchar(50)  
,Vertical varchar(50)  
...
BEGIN  
 ...
 SELECT DISTINCT A.Parent_Accountid,A.Parent_AccountName,A.SBU, Vertical,SDM_ID ,SDM_Name ,SDD_ID ,SDD_Name         
 ...
 INNER JOIN ADPR.Project_Compliance P ON A.Project_ID=P.EsaProjectid  
 WHERE Department_Name='ADM' OR Department_Name LIKE  '%AVM%' or    
 Department_Name LIKE  '%ADM-%' or Department_Name LIKE  'ADM%'     
  
   INSERT INTO  #Associate_Final_AVM_Below_M   
 SELECT DISTINCT A.Parent_Accountid,A.Parent_AccountName,A.SBU, Vertical,SDM_ID ,SDM_Name ,SDD_ID ,SDD_Name         
 ,Project_ID ,[Project Name],associate_id ,Associate_Name, Allocation_Startdate ,Allocation_Enddate ,Allocation_Percentage,              
 ESA_FTE_Count ,Available_Hours ,DE_Inscope ,Department_Name ,Job_Code,a.Designation          
 FROM [ADPR].[Associate_Allocation_Raw] a join #Temp_BM_Applns b on a.Associate_id=b.AssociateID   
    INNER JOIN ADPR.Project_Compliance P ON A.Project_ID=P.EsaProjectid  
 WHERE Department_Name='ADM' OR Department_Name LIKE  '%AVM%' or    
 Department_Name LIKE  '%ADM-%' or Department_Name LIKE  'ADM%'   
END  
ELSE BEGIN  
 ...
 WHERE  Department_Name LIKE @ReportType+'%'  
...
DELETE  from #Associate_Final_AVM_Below_M where associate_id='323477'          
...
FROM #Associate_Final_AVM_Below_M AF      
...
LEFT JOIN [ADPR].Input_Data_AssociateRAW AD      
 ON PM.EsaProjectID = ad.EsaProjectID AND AD.OnBoardStatus='Onboarded'     
...
FROM #TEMPR LG      
 ...
 LEFT JOIN #Associate_Final_AVM_Below_M AF ON PM.EsaProjectID = AF.Project_ID      
...
SELECT DISTINCT      
 ...
 FROM #Associate_Final_AVM_Below_M A    
...
INSERT INTO #Associate_Summary      
 ...
 join [ADPR].Input_Data_AssociateRAW BU      
  ON af.EsaProjectID = bU.EsaProjectID AND BU.OnBoardStatus='Onboarded'    
 LEFT JOIN #Total_Effort B      
  ON IsNull(af.Parent_Accountid,0) = IsNull(b.Parent_Accountid,0)       
...
Parent_Accountid Char(15),      
Parent_AccountName varchar (100),      EsaProjectID Char(15) Not Null,      
Projectname varchar(100),      
EmployeeID varchar(15),      
EmployeeName varchar(100),      
IsNonESAAuthorized bit,      
...
from #AssociateActual_Final_AVM A      
JOIN [ADPR].Input_Data_AssociateRAW C ON a.EsaProjectID=C.EsaProjectID AND C.OnBoardStatus='Onboarded'   
JOIN ADPR.Project_Compliance D on A.EsaProjectid=D.EsaProjectID  
where C.DE_Inscope='In scope'  --and Parent_Accountid in('2000004','2000522')    
GROUP BY A.Parent_Accountid,A.Parent_AccountName,EmployeeID,Department_Name,Vertical,C.PracticeOwner,C.MARKET_BU   
...
FROM #Account_Compliance_AVM ORDER BY Associate_Compliance_Percent DESC   
...
ISNULL(((SUM(A.Actual_Effort)) / NULLIF(SUM(A.Available_Hours), 0) * 100), 0) from #AssociateActual_Final_AVM A      
JOIN [ADPR].Input_Data_AssociateRAW C ON a.EsaProjectID=C.EsaProjectID AND C.OnBoardStatus='Onboarded'       
...
SELECT DISTINCT      
 ...
 --IF(@ReportType='ADM')  
 --BEGIN  
 -- DELETE FROM #Project_Compliance_AVM WHERE ISNULL(ESA_All_FTE,0)<5  
 --END  
...
FROM #Project_Compliance_AVM ORDER BY avmAssociate_Project_Compliance_Percent DESC     
...
SELECT DISTINCT A.SBU,a.EsaProjectID,A.EmployeeID,A.Department_Name,sum(A.Avaialble_FTE_Below_M),sum(A.Available_Hours),sum(A.MPS_Effort), sum(A.Work_Profile_AD),     
Sum(A.MAS_Effort),sum(A.Actual_Effort),ISNULL(((SUM(A.Actual_Effort)) / NULLIF(SUM(A.Available_Hours), 0) * 100), 0)   
from #AssociateActual_Final_AVM A      
JOIN [ADPR].Input_Data_AssociateRAW C ON a.EsaProjectID=C.EsaProjectID  AND C.OnBoardStatus='Onboarded'  
JOIN ADPR.Project_Compliance pc on pc.EsaProjectid=a.EsaProjectID  
...
SBU nvarchar (50),      EmployeeID varchar(15),      
...
INSERT INTO #SBU_Compliance_AVM      
 ...
 select SBU,sum(ESA_FTE_80_BU) as 'ESA_FTE_80_BU' into #Associate_80_SBU_Total  from #Associate_80_SBU group by SBU  
    
 update a set a.ESA_FTE_80=b.ESA_FTE_80_BU  
 from #SBU_Compliance_AVM a  
 join #Associate_80_SBU_Total b on a.SBU = b.SBU  
  
   
 select SBU,sum(Avaialble_FTE_Below_M) as 'Avaialble_FTE_Below_M' into #Associate_BUcompliance_AVM_all_Total from #Associate_BUcompliance_AVM_all group by SBU  
 select * from #Associate_BUcompliance_AVM_all_Total  
  
 update a set   
 a.Associate_Compliance_Percent= ISNULL((ISNULL(a.ESA_FTE_80,0) / NULLIF(b.Avaialble_FTE_Below_M,0))*100,0)  
 from #SBU_Compliance_AVM a  
 join #Associate_BUcompliance_AVM_all_Total b on a.SBU = b.SBU  
...
JOIN [ADPR].Input_Data_AssociateRAW C ON A.parent_accountid=C.parentaccountid and   
a.EsaProjectID=C.EsaProjectID  AND C.OnBoardStatus='Onboarded'     
...
INSERT INTO #SBU_Compliance_AVM_ALL_Grade      
 ...
 select SBU,sum(ESA_FTE_80_BU) as 'ESA_FTE_80_BU' into #Associate_80_SBU_ALL_Grade_Total  from #Associate_80_SBU_ALL_Grade group by SBU  
 ...
 select SBU,sum(Avaialble_FTE_Below_M) as 'Avaialble_FTE_Below_M' into #Associate_BUcompliance_AVM_ALL_Grade_Total from #Associate_BUcompliance_AVM_ALL_Grade group by SBU  
  
 update a set   
 a.Associate_Compliance_Percent= ISNULL((ISNULL(a.ESA_FTE_80,0) / NULLIF(b.Avaialble_FTE_Below_M,0))*100,0)  
 from #SBU_Compliance_AVM_ALL_Grade a  
 join #Associate_BUcompliance_AVM_ALL_Grade_Total b on a.SBU = b.SBU  
...
END TRY    
 ...
 SELECT @ErrorMessage = ERROR_MESSAGE()    
  --INSERT Error        
  EXEC [AppVisionLens].[dbo].AVL_InsertError '[ADPR].[AVM_AssociateData_WeekAndMonth] ', @ErrorMessage, '',''    
  RETURN @ErrorMessage    
  END CATCH       
