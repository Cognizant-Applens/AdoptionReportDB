...
CREATE Procedure [ADPR].[CDB_ACCOUNT_PROJECT_SUMMARY_RAW] 
(  
@startdate datetime,  
@endDate datetime,  
@mode varchar(200)  
...
SELECT  DISTINCT  
   ...
   ,ISNULL(B.AVM_ESA_FTE,0) AS 'CDB FTE'  
   ...
   ,ISNULL(B.[AVM FTE with TSC %>80],0) AS 'CDB FTE with TSC %>80'  
   ,ISNULL(a.[Available Hours],0) AS 'Available Hours (All)'  
   ,ISNULL(B.Available_Hours,0) As 'Available Hours (CDB)'  
   ,ISNULL(A.[Actual Effort],0) as 'Actual Effort (All)'  
   ,ISNULL(B.Actual_Effort,0) as 'Actual Effort (CDB)'  
   ...
   ,ISNULL(B.[Effort Account Compliance% (AVM)],0) as 'Account Effort Compliance% (CDB)'  
   ,ISNULL(B.AVM_Associate_Compliance_Percent,0) AS 'Account Associate Compliance% (CDB)'  
...
SELECT  DISTINCT 
    ...
    ,ISNULL(B.[AVM #FTE with TSC %>80],0) AS 'CDB #FTE with TSC %>80'  
    ,ISNULL(a.[Available Hours],0) AS 'Available Hours (All)'  
    ,ISNULL(B.[Available Hours],0) As 'Available Hours (CDB)'  
    ,ISNULL(A.[Actual Effort],0) as 'Actual Effort (All)'  
    ,ISNULL(B.[Actual Effort],0) as 'Actual Effort (CDB)'  
    ...
    ,ISNULL(B.[Effort Project Compliance% (AVM)],0) as 'Project Effort Compliance% (CDB)'  
    ,ISNULL(B.AVMAssociate_Project_Compliance_Percent,0) AS 'Project Associate Compliance% (CDB)' 
...
END TRY  
 ...
 SELECT @ErrorMessage = ERROR_MESSAGE()  
  --INSERT Error      
  EXEC [AppVisionLens].dbo.AVL_InsertError '[ADPR].[CDB_ACCOUNT_PROJECT_SUMMARY_RAW]  ', @ErrorMessage, '',''  
  RETURN @ErrorMessage  
  END CATCH     
