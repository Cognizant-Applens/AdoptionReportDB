CREATE PROCEDURE [ADPR].[Input_Data_Refresh_Associate] --'ADM'
...
TRUNCATE table [ADPR].[Input_Excel_Associate]

IF @ReportType = 'ADM' BEGIN
	INSERT into [ADPR].[Input_Excel_Associate] (EsaProjectID,PracticeOwner,DE_Inscope,SBU,[CHILDPROJECT])
	SELECT distinct EsaProjectID,PracticeOwner,DE_Inscope,SBU,[CHILDPROJECT] FROM [ADPR].[Associate_Projects] 
END
--Non ADM Report
ELSE BEGIN
	INSERT into [ADPR].[Input_Excel_Associate] (EsaProjectID,PracticeOwner,DE_Inscope,SBU,[CHILDPROJECT])
	SELECT distinct EsaProjectID,PracticeOwner,DE_Inscope,SBU,[CHILDPROJECT] FROM [ADPR].[NonADM_EligibleProjects] WHERE ReportType=@ReportType
	AND EsaProjectID NOT IN (SELECT EsaProjectID FROM ADPR.NonADM_NotEligibleProjects WHERE IsDeleted=0 AND ValidTillDate>=GETDATE())
END

UPDATE EX set EX.DartOrApp=2  from [ADPR].[Input_Excel_Associate] EX 
...
UPDATE EX set EX.IsConfigured=1  from [ADPR].[Input_Excel_Associate] EX 
join [AppVisionLens].AVL.MAS_ProjectMaster PM on Pm.EsaProjectID=EX.EsaProjectID  and  PM.IsDeleted=0 and PM.IsESAProject=1 --and (pm.IsMigratedFromDART=2 or pm.IsMigratedFromDART is null) 
...
UPDATE A set A.DartOrApp=3 from [ADPR].[Input_Excel_Associate] A
...
from [ADPR].[Input_Excel_Associate] AIE where  AIE.DartOrApp=2 and AIE.DartOrApp=2
...
TRUNCATE table [ADPR].[Input_Data_AssociateRAW]
IF @ReportType = 'ADM' BEGIN
	Insert into [ADPR].[Input_Data_AssociateRAW]
	...
	b.Dept_Name,b.PROJECT_MANAGER,b.PMName,b.Parent_Account_ID,b.Parent_Account_Name,
	AP.MARKET,AP.MARKET_BU,'',E.OnboardStatus
	from [ADPR].[Input_Excel_Associate] A 
	Inner Join #tmp_PAcc_ID B on A.ESAProjectID= B.Project_ID 
	inner join [ADPR].[Associate_Projects] AP on B.Project_ID =Ap.EsaProjectID
	inner join [ADPR].[AdoptionTotalEligibleProjects] E On E.EsaProjectID=A.ESAProjectID
...
ELSE BEGIN
	...
	Select distinct 	A.PracticeOwner, C.CTS_Vertical as Project_Owning_Practice, A.DE_Inscope, b.PROJECT_ID,b.Project_Name,b.project_Owner,b.POName,b.Deliverymanagerid,b.DMName,
	b.Dept_Name,b.PROJECT_MANAGER,b.PMName,b.Parent_Account_ID,b.Parent_Account_Name,
	EP.MARKET,EP.MARKET_BU,'','Onboarded' AS OnboardStatus
	from [ADPR].[Input_Excel_Associate] A 
	Inner Join #tmp_PAcc_ID B on A.ESAProjectID= B.Project_ID 
	inner join ADPR.NonAdm_EligibleProjects EP on B.Project_ID =EP.EsaProjectID AND EP.EsaProjectID NOT IN (SELECT EsaProjectID FROM ADPR.NonADM_NotEligibleProjects WHERE IsDeleted=0 AND ValidTillDate>=GETDATE())
	Inner Join #TempPracOwningBU C on A.ESAProjectID= C.ProjectID

	UPDATE A SET A.OnboardStatus='Exempted'
    FROM ADPR.Input_Data_AssociateRAW A
    JOIN #ExemptionProjects B ON A.EsaProjectID=B.EsaProjectId
END


Delete from [ADPR].[Input_Data_AssociateRAW] where EsaProjectID in (Select EsaProjectID from [AppVisionLens].AVL.MAS_ProjectMaster where IsODCRestricted = 'Y')

Update [ADPR].[Input_Data_AssociateRAW] SET PracticeOwner = 'HEALTHCARE' WHERE PracticeOwner = 'XEROX'

Update [ADPR].[Input_Data_AssociateRAW] SET ProjectOwningPractice = 'HEALTHCARE' WHERE ProjectOwningPractice = 'XEROX'


select distinct A.EsaProjectID,C.AttributeValueID as 'ScopeName' , 'Scp' As 'Scope'into #ProjectScope from [ADPR].[Input_Data_AssociateRAW] A
...
from [ADPR].[Input_Data_AssociateRAW] A join #ProjectScope B on A.EsaProjectID=B.EsaProjectID where B.ScopeName in ('1')

Update A set PROJECTSCOPE='AD'
from [ADPR].[Input_Data_AssociateRAW] A join #ProjectScope B on A.EsaProjectID=B.EsaProjectID where B.ScopeName in ('4')

Update A set PROJECTSCOPE='AD + AVM'
from [ADPR].[Input_Data_AssociateRAW] A join #ProjectScope B on A.EsaProjectID=B.EsaProjectID where B.ScopeName in ('2')
...
from [ADPR].[Input_Data_AssociateRAW] A join #ProjectScope B on A.EsaProjectID=B.EsaProjectID where B.ScopeName in ('3')
and A.PROJECTSCOPE <> ''
...
,[MARKET_BU]
,[PROJECTSCOPE]
from [ADPR].[Input_Data_AssociateRAW] 
...
BEGIN CATCH
	...
	SELECT @ErrorMessage = ERROR_MESSAGE()
		--INSERT Error    
		EXEC [AppVisionLens].dbo.AVL_InsertError '[ADPR].[Input_Data_Refresh_Associate]', @ErrorMessage, '',''
		RETURN @ErrorMessage
END CATCH   
